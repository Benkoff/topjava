...
... сollect(
        Collector.of(
                    supplier(),   //метод_инициализации_аккумулятора,
                    accumulator(),//метод_обработки_каждого_элемента,
                    combiner(),   //метод_соединения_двух_аккумуляторов,
                    finisher()    //метод_последней_обработки_аккумулятора
        ));
```
